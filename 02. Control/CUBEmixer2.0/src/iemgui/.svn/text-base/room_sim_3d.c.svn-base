...
iemgui written by Thomas Musil, Copyright (c) IEM KUG Graz Austria 2000 - 2004 */
...
static void room_sim_3d_out_para(t_room_sim_3d *x)
{
	...
	SETFLOAT(x->x_at+2, (float)(h2 - x->x_pix_src_y[0])/x->x_cnvrt_roomlx2pixh);
	SETFLOAT(x->x_at+3, (float)(w2 - x->x_pix_src_x[0])/x->x_cnvrt_roomlx2pixh);
	SETFLOAT(x->x_at+4, (float)(x->x_pix_src_z[0])/x->x_cnvrt_roomlx2pixh);
	outlet_list(x->x_out_para, &s_list, 5, x->x_at);
	for(i=1; i<=n; i++)
	{
		SETFLOAT(x->x_at, (float)i);
		SETSYMBOL(x->x_at+1, x->x_s_src_xyz);
		SETFLOAT(x->x_at+2, (float)(h2 - x->x_pix_src_y[i])/x->x_cnvrt_roomlx2pixh);
		SETFLOAT(x->x_at+3, (float)(w2 - x->x_pix_src_x[i])/x->x_cnvrt_roomlx2pixh);
		SETFLOAT(x->x_at+4, (float)(x->x_pix_src_z[i])/x->x_cnvrt_roomlx2pixh);
		outlet_list(x->x_out_para, &s_list, 5, x->x_at);
	}
}
...
static void room_sim_3d_draw_update(t_room_sim_3d *x, t_glist *glist)
{
	if(glist_isvisible(glist))
	{
		...
		dx = -(int)((float)x->x_pix_rad*(float)sin(x->x_rho_head*0.0174533f) + 0.49999f);
		dy = -(int)((float)x->x_pix_rad*(float)cos(x->x_rho_head*0.0174533f) + 0.49999f);
...
static void room_sim_3d_save(t_gobj *z, t_binbuf *b)
{
	...
	binbuf_addv(b, "ifffi", x->x_n_src, x->x_cnvrt_roomlx2pixh, x->x_rho_head, x->x_r_ambi, x->x_fontsize);
	c = x->x_gui.x_bcol;
...
static void room_sim_3d_motion(t_room_sim_3d *x, t_floatarg dx, t_floatarg dy)
{
	...
	else if(sel == 0)
	{
		...
		sys_vgui(".x%x.c coords %xHEAD2 %d %d %d %d\n",
			canvas, x, xpos+x->x_pix_src_x[0]-rad2, ypos+x->x_pix_src_y[0]-rad2,
				xpos+x->x_pix_src_x[0]+rad2-1, ypos+x->x_pix_src_y[0]+rad2-1);
		ddx = -(int)((float)pixrad*(float)sin(x->x_rho_head*0.0174533f) + 0.49999f);
		ddy = -(int)((float)pixrad*(float)cos(x->x_rho_head*0.0174533f) + 0.49999f);
		sys_vgui(".x%x.c coords %xNOSE %d %d %d %d\n",
			canvas, x, xpos+x->x_pix_src_x[0], ypos+x->x_pix_src_y[0],
...
static void room_sim_3d_set_src_xyz(t_room_sim_3d *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	float roomx2=0.5f*x->x_room_x, roomy2=0.5f*x->x_room_y, roomz=x->x_room_z;
	int i, n=x->x_n_src;
	int pixrad=x->x_pix_rad;
	int xpos=text_xpix(&x->x_gui.x_obj, x->x_gui.x_glist);
	int ypos=text_ypix(&x->x_gui.x_obj, x->x_gui.x_glist);
...
static void room_sim_3d_pix_per_m_ratio(t_room_sim_3d *x, t_floatarg ratio)
{
	...
	for(i=0; i<=n; i++)
	{
		x->x_pix_src_x[i] = (int)((float)x->x_pix_src_x[i]*rr + 0.49999f);
		x->x_pix_src_y[i] = (int)((float)x->x_pix_src_y[i]*rr + 0.49999f);
		x->x_pix_src_z[i] = (int)((float)x->x_pix_src_z[i]*rr + 0.49999f);
	}
	(*x->x_gui.x_draw)(x, x->x_gui.x_glist, IEM_GUI_DRAW_MODE_MOVE);
	canvas_fixlinesfor(glist_getcanvas(x->x_gui.x_glist), (t_text*)x);
}
