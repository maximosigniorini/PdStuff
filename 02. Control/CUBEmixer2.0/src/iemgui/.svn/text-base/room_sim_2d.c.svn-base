...
iemgui written by Thomas Musil, Copyright (c) IEM KUG Graz Austria 2000 - 2003 */
...
{
	float xsrc, ysrc;
	float roomx2=0.5f*x->x_room_x, roomy2=0.5f*x->x_room_y;
	int i, n=x->x_n_src;
	int xpos=text_xpix(&x->x_gui.x_obj, x->x_gui.x_glist);
	int ypos=text_ypix(&x->x_gui.x_obj, x->x_gui.x_glist);
	int w2=x->x_gui.x_w/2, h2=x->x_gui.x_h/2;
...
static void room_sim_2d_room_dim(t_room_sim_2d *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	x->x_gui.x_h = (int)(x->x_cnvrt_roomlx2pixh * (float)x->x_room_x + 0.49999f);
	x->x_gui.x_w = (int)(x->x_cnvrt_roomlx2pixh * (float)x->x_room_y + 0.49999f);
